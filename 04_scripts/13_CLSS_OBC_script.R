...
temporal_extent <- 32
SE <- c(3, 4, 5, 6, 7, 8, 9, 10)
IC <- c(5)

for(spatial_extent in SE){
  for(inv_coupling in IC){
    ...
    if(spatial_extent == 4 && debug == TRUE)
      r_i <- c(1, sqrt(2), 2, sqrt(5), sqrt(8), sqrt(10), sqrt(18), sqrt(32))
    if(spatial_extent == 5)
    ...
    if(spatial_extent >= 9) ##changed in the script
      r_i <- c(1, sqrt(2), sqrt(5), sqrt(8), 3, sqrt(10), sqrt(18), 5, sqrt(32), sqrt(40), sqrt(72), sqrt(45), sqrt(98), sqrt(50), sqrt(128))
    
    extra = "_0"
    
    folder <- paste0("debug_pascal_OBC_", inv_coupling, "_", spatial_extent, "_", temporal_extent, extra)
    ...
    lims1 <- rep(list(c(0, 0.2)), length(r_i)%/%2)
